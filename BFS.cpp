...
ll n,m,visited[100000],level[100000],parent[100000];
vector<ll> v[100000];
...
void bfs(ll startingNode)
{
	...
	while(q.size())
	{
		ll beginingOfQueue = q.front();
		FOR(i,0,v[beginingOfQueue].size())
		{
			if(  visited[v[beginingOfQueue][i]]==0)
			{
				q.push(v[beginingOfQueue][i]);
				visited[v[beginingOfQueue][i]]=1;
				parent[v[beginingOfQueue][i]]=beginingOfQueue;
				level[v[beginingOfQueue][i]]=level[beginingOfQueue]+1;
			}
		}
		q.pop();
	}
